...
系统已经能回答一部分关键问题：当天有没有主线、候选票属于什么模式、是否可追强、是否有风险、你的实际操作最终赚亏多少。已有报告也说明系统已经开始做“策略验证”，比如 [mainline_relay_validation](/Users/x6-mac/WWW/stock/outputs/mainline_relay_validation/summary.md) 已经把真实买入和次日10点前表现接起来了。
目前报告里的关键现象是：系统不是完全找不到机会，而是“强票里混入了不少收盘守不住的票”。所以后期重点不应该只是继续加提示，而是验证每个提示到底有没有用。
最大断点
现在最缺的是“不可变的信号留痕”。
也就是说，某天 09:40 系统给了什么候选、什么核心分、什么买入状态、什么市场环境，应该原样保存下来。否则以后规则改了，再回看历史，会出现“现在重算出来的系统判断”和“当时真实看到的系统判断”不一致。
这会影响你最想知道的问题：
“可追强”到底比“重点观察”胜率高多少？
“主线共振弱”是不是应该硬过滤？
弱市退潮时关闭候选池是否真的减少亏损？
我没买推荐票时，是系统错过了好票，还是我偏离系统导致亏损？
后期发展建议
我建议主系统先分四步走：
做“信号留痕”
新增一个每日/每快照的信号快照表，保存策略版本、参数、市场环境、候选池、每只股票当时的系统判断。

做“策略验证”页面
在“操作策略”下面再加一个“策略验证/胜率复盘”页面，按模式、买入状态、风险标签、主线、快照时间、市场环境统计胜率、平均收益、最大回撤、次日10点前红盘率、实际盈亏。

做“买卖一体决策”
现在买入提示已经有雏形，下一步要解决“继续持有 vs 卖出换股”。这个要比较当前持仓的系统状态和新候选池强度。

做“规则治理”
每次改规则都要有版本号和回放结果。不要靠感觉调参数，而是形成：提出规则 → 历史验证 → 小样本实盘观察 → 再升级为默认规则。

我的优先级建议
下一步最值得先做的是“策略验证页 + 信号快照表”。这一步做完，你每天就不只是看推荐，而是能持续回答：这个版本的系统到底有没有提高你的胜率，哪些提示该信，哪些提示该降权，哪些市场环境应该直接空仓。
机器学习那块先不用急。等主系统把真实交易、系统信号、结果收益沉淀干净之后，ml_predict 才有高质量训练材料。现在先把规则系统的验证闭环打牢，更适合你当前的操盘方式。